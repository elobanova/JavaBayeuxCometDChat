(function ($) {
	$.cometChat = {
		...
		broadcastmsg : function (textMessage) {

			if (!textMessage || !textMessage.length)
				return;

			$.cometd.publish('/service/commonchat', {
				room : '/chat/demo',
				user : $.cometChat.loginUserName,
				chat : textMessage
			});
		...
		commonchat : function (message) {
			var userNameCssClass = "fromUserName",
			textMessageCssClass = "fromMessage",
			commonChatWindow = document.getElementById($.cometChat.commonChatId);
			var sb = [];

			for (var i = 0; i < message.data.length; i++) {
				sb[sb.length] = '<span class="' + userNameCssClass + '">' + message.data[i].user + ':' + '</span>';
				sb[sb.length] = '<span class="' + textMessageCssClass + '">' + message.data[i].chat + '</span><br/>';
			}
			$('#' + $.cometChat.commonChatId).html(sb.join(""));
		},
		...
		_unsubscribe : function () {
			...
			if ($.cometChat._commonChatSubscription) {
				$.cometd.unsubscribe($.cometChat._commonChatSubscription);
			}
			$.cometChat._commonChatSubscription = null;
		},
		...
		_subscribe : function () {
			...
			$.cometChat._commonChatSubscription = $.cometd.subscribe('/commonchat/demo', $.cometChat.commonchat);
		},
		...
		onLoad : function (config) {
			...
			$.cometChat.commonChatId = config.commonChatId;
